...
'*** Jacob 5/17/2011 - These privateprofilestring functions are defined in PrivateProfileModule
...
Public Declare Function InternetSetStatusCallback Lib "wininet.dll" (ByVal pub_lngInternetSession As Long, ByVal lpfnInternetCallback As Long)
...
Declare Function FtpGetFile Lib "wininet.dll" Alias "FtpGetFileA" (ByVal hFtpSession As Long, ByVal lpszRemoteFile As String, ByVal lpszNewFile As String, ByVal fFailIfExists As Long, ByVal dwFlagsAndAttributes As Long, ByVal dwFlags As Long, ByVal dwContext As Long) As Boolean
Declare Function FtpPutFile Lib "wininet.dll" Alias "FtpPutFileA" (ByVal hFtpSession As Long, ByVal lpszLocalFile As String, ByVal lpszNewRemoteFile As String, ByVal dwFlags As Long, ByVal dwContext As Long) As Boolean
...
Public Function funcFTPShowError()
...
Public Function funcFTPGetError() As String
    Dim lErr As Long, sErr As String, lenBuf As Long
    'get the required buffer size
    InternetGetLastResponseInfo lErr, sErr, lenBuf
    'create a buffer
    ...
    InternetGetLastResponseInfo lErr, sErr, lenBuf
    'show the last response info
    funcFTPGetError = "ERROR " + CStr(lErr) + ": " + sErr
End Function

Public Function INTERNET_STATUS_CALLBACK( _
...
'
'  *** AN EXAMPLE OF USING CALLBACK FOR ASYNCH INTERNET CONNECTIONS
' *** NOTE:  SOME STUFF MAY BE MISSING FOR THIS EXAMPLE TO WORK
'
'  Dim dwRead As Long
...
Public Function funcFTPPutFile(ByVal strFTPserver As String, _
                                             ByVal intFTPport As Integer, _
                                             ByVal strUser As String, _
                                             ByVal strPass As String, _
                                             ByVal strLocalfile As String, _
                                             ByVal strRemotefile As String) As Boolean
        ...
        Dim strError As String
         ...
         frmFTP.lblRESPONSE = " -> Establish FTP Session to " & strFTPserver & ":" & intFTPport
        lngSession = InternetOpen(App.EXEName, INTERNET_OPEN_TYPE_DIRECT, vbNullString, vbNullString, INTERNET_FLAG_NO_CACHE_WRITE)
        
'        If AsyncMode = True Then
'          result = InternetSetStatusCallback(CLng(session), AddressOf INTERNET_STATUS_CALLBACK)
'        End If

        lngServer = InternetConnect(lngSession, strFTPserver, intFTPport, strUser, strPass, INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, 0)
        
        If lngServer = 0 Then
            frmFTP.lblRESPONSE = " *  ERROR Connecting to Server: " & strFTPserver
            funcQuickMessage "SHOW", "ERROR Connecting to Server: " & vbCrLf & vbCrLf & strFTPserver
        ...
        frmFTP.lblRESPONSE = " -> Begin Transfer for File: " & strRemotefile
        lngRet = FtpPutFile(lngServer, strLocalfile, strRemotefile, FTP_TRANSFER_TYPE_BINARY, 0)
        
        If lngRet = 0 Then
            frmFTP.lblRESPONSE = " *  ERROR Transferring file: " & strRemotefile
            frmFTP.lblRESPONSE = funcFTPGetError()
           frmFTP.lblRESPONSE = "  "
            strError = "ERROR Transferring file: " & strRemotefile & vbCrLf & funcFTPGetError()
            funcQuickMessage "SHOW", strError
            bolErrorOccured = True

        Else
            bolErrorOccured = False
           frmFTP.lblRESPONSE = "  -> Transfer Successful"
           frmFTP.lblRESPONSE = "  "
        End If
        
        
CLOSE_INTERNET:
        InternetCloseHandle lngServer
        InternetCloseHandle lngSession
        
        funcFTPPutFile = bolErrorOccured
