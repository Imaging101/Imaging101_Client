...
Begin VB.Form frmJMHDocuments 
   Caption         =   "Document List"
   ClientHeight    =   7020
   ClientLeft      =   6465
   ClientTop       =   255
   ClientWidth     =   3195
   Icon            =   "frmJMHDocuments.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7020
   ScaleWidth      =   3195
   ...
   Begin MSDataListLib.DataCombo DataComboAreaList 
      Bindings        =   "frmJMHDocuments.frx":0442
   ...
   End
End
Attribute VB_Name = "frmJMHDocuments"
...
''    End If
    ...
    AdodcDocList.ConnectionString = RegConnectionString
    AdodcDocList.RecordSource = ""
    
    AdodcAreaList.ConnectionString = RegConnectionString
...
'    On Error Resume Next
    Me.Top = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.Top", RegFileName)
    Me.Left = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.Left", RegFileName)
    Me.Width = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.Width", RegFileName)
    Me.Height = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.Height", RegFileName)
    Me.Caption = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.Caption", RegFileName)
    If Me.Caption = "" Then Me.Caption = "Document List"
    
    strDOCGROUP = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.DOCGROUP", RegFileName)
    strDOCTYPE = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.DOCTYPE", RegFileName)
    strFIELDAFTERCLICK = VBGetPrivateProfileString(RegAppname, "frmJMHDocuments.FIELDAFTERCLICK", RegFileName)
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    ...
    Else
        ...
        If Me.Top > 0 And Me.Left > 0 Then
            Result = WritePrivateProfileString(RegAppname, "frmJMHDocuments.Top", Me.Top, RegFileName)
            Result = WritePrivateProfileString(RegAppname, "frmJMHDocuments.Left", Me.Left, RegFileName)
            Result = WritePrivateProfileString(RegAppname, "frmJMHDocuments.Width", Me.Width, RegFileName)
            Result = WritePrivateProfileString(RegAppname, "frmJMHDocuments.Height", Me.Height, RegFileName)
            Result = WritePrivateProfileString(RegAppname, "frmJMHDocuments.Caption", Me.Caption, RegFileName)
        End If
...
''                        funcListView_SetColumnWidth frmJMHDocuments.ListView1, 1, AdodcDocList.Recordset.Fields.Item(intListIndex).Value
                    End If
                Else
...
Private Sub DataComboAreaList_Change()
        ...
        If Trim(DataComboAreaList.Text) <> "*ALL*" Then
            AdodcDocList.RecordSource = AdodcDocList.RecordSource & " AND AREA = '" & DataComboAreaList.Text & "'"
        End If
        AdodcDocList.RecordSource = AdodcDocList.RecordSource & " order by FORMDESC"
        AdodcDocList.Refresh
